...
GenLatticeNode* buildGenLattice(std::multimap<uint32_t, ClosedIS*>* ClosureList, TIDList* TList) {
		// Step 1: Initialize the root node (empty set)
		ClosedIS* emptyClos = nullptr;
		for (auto& entry : *ClosureList) {
				if (!entry.second->deleted && entry.second->itemset.empty()) {
						emptyClos = entry.second;
						break;
				}
		}
		if (!emptyClos) {
				std::set<uint32_t> emptySet;
				emptyClos = new ClosedIS(emptySet, TList->TransactionList.size(), ClosureList);
		}
		...
		// Step 3: Sort generators by size, decreasing support
		std::sort(generators.begin(), generators.end(),
				[](const auto& a, const auto& b) {
						if (a.first.size() != b.first.size()) {
								return a.first.size() < b.first.size(); // Increasing size (size-1 first)
						}
						if (a.second->support != b.second->support) {
								return a.second->support > b.second->support; // Decreasing support
						}
						return a.first < b.first; // Lexicographic for tie-breaking
				});

		// Step 4: Initialize data structures
		std::map<std::set<uint32_t>, GenLatticeNode*> nodeMap; // Maps generators to nodes
		std::map<std::set<uint32_t>, std::vector<GenLatticeNode*>> equivClasses; // Maps closures to their generators
		nodeMap[root->generator] = root;
		equivClasses[emptyClos->itemset] = {root};

		// Step 5: Build the lattice and equivalence classes
		for (size_t i = 0; i < generators.size(); ++i) {
				auto& genPair = generators[i];
				std::set<uint32_t> gen = genPair.first;
				ClosedIS* clos = genPair.second;

				// Create new node
				uint32_t lastItem = gen.empty() ? -1 : *gen.rbegin();
				GenLatticeNode* newNode = new GenLatticeNode(lastItem, gen, clos->support, clos, nullptr);

				// Find parent: node whose closure includes gen and has equal or higher support
				GenLatticeNode* parent = nullptr;
				uint32_t maxSupport = 0;
				for (auto& candidatePair : nodeMap) {
						GenLatticeNode* candidate = candidatePair.second;
						if (candidate->closure->support < clos->support) continue;
						if (std::includes(candidate->closure->itemset.begin(), candidate->closure->itemset.end(),
															gen.begin(), gen.end())) {
								if (!parent || candidate->closure->support > maxSupport ||
										(candidate->closure->support == maxSupport && candidate->generator.size() < parent->generator.size())) {
										parent = candidate;
										maxSupport = candidate->closure->support;
								}
						}
				}
				if (!parent) {
				...
				if (!parent->fils) {
						...
						GenLatticeNode* sibling = parent->fils;
						while (sibling->frere) {
								sibling = sibling->frere;
						}
						sibling->frere = newNode;
				}

				// Add to nodeMap
				nodeMap[gen] = newNode;

				// Add to equivalence class (same closure implies same equivalence class)
				auto& equivList = equivClasses[clos->itemset];
				if (equivList.empty()) {
						equivList.push_back(newNode);
				} else {
						equivList.push_back(newNode);
				}
		}

		// Step 6: Link equivalence classes (circular list)
		for (auto& equivPair : equivClasses) {
				auto& nodes = equivPair.second;
				for (size_t i = 0; i < nodes.size(); ++i) {
						if (nodes[i]) {
								nodes[i]->equiv_next = nodes[(i + 1) % nodes.size()];
						}
				}
		}

		// Step 7: Link predecessors and successors using Proposition b
		for (size_t i = 0; i < generators.size(); ++i) {
				auto& genPairX = generators[i];
				std::set<uint32_t> genX = genPairX.first;
				ClosedIS* closX = genPairX.second;
				GenLatticeNode* nodeX = nodeMap[genX];

				// Only check generators with higher support (sorted order helps)
				for (size_t j = 0; j < i; ++j) { // j < i ensures Supp(Y) > Supp(X)
						auto& genPairY = generators[j];
						std::set<uint32_t> genY = genPairY.first;
						ClosedIS* closY = genPairY.second;
						if (closY->support <= closX->support) continue; // Proposition b requires Supp(X) < Supp(Y)
						GenLatticeNode* nodeY = nodeMap[genY];

						// Proposition b: If Supp(X) < Supp(Y) and Supp(X) = Supp(X ∪ Y)
						std::set<uint32_t> unionXY = genX;
						unionXY.insert(genY.begin(), genY.end());
						uint32_t unionSupp = TList->supp_from_tidlist(unionXY);
						if (closX->support == unionSupp) {
								nodeX->preds.insert(nodeY);
								nodeY->succs.insert(nodeX);
						}
				}
		}
...
void printGenLattice(GenLatticeNode* node, std::ostream& out, int level) {
	...
	// Equivalence class
	out << " [Equiv: ";
	GenLatticeNode* current = node;
	do {
			out << "{ ";
			for (auto item : current->generator) {
					out << item << " ";
			}
			out << "}";
			current = current->equiv_next;
			if (current != node) out << ", ";
	...
	// Closure
	out << " Closure: { ";
	for (auto item : node->closure->itemset) {
			out << item << " ";
	}
	out << "}";
	...
	for (GenLatticeNode* child = node->fils; child; child = child->frere) {
			printGenLattice(child, out, level + 1);
	}
}

void releaseGenLattice(GenLatticeNode* node) {
	if (!node) return;
	GenLatticeNode* child = node->fils;
	while (child) {
			GenLatticeNode* next = child->frere;
			releaseGenLattice(child);
			child = next;
	}
	node->preds.clear();
	node->succs.clear();
...
void descend(GenNode* n, std::set<uint32_t> X, std::set<uint32_t> t_n, std::multimap < uint32_t, ClosedIS* >* fGenitors, std::multimap<uint32_t, ClosedIS*>* ClosureList, std::vector<ClosedIS*>* newClosures, TIDList* TList, GenNode* root) {
	if (n->item != static_cast<uint32_t>(1 << 31)) { // (1 << 31) is reserved for the root, so n->item = (1 << 31) actually means n is the empty set
		X.insert(n->item);
	}
...
void computeJumpers(GenNode* n, std::set<uint32_t> t_n, std::vector<ClosedIS*>* newClosures, TIDList* TList, GenNode* root, std::multimap<uint32_t, ClosedIS*>* ClosureList) {
			...
			//for (std::map<uint32_t, GenNode*>::const_iterator right = std::next(left); right != n->succ->end(); right++) {
				...
				//if (t_n.find(right->second->item) != t_n.end()) {
					...
					if (leftN->succ->find(rightN->item) == leftN->succ->end()) {
						...
						if (support == static_cast<int>(minSupp)) {
							bool isGen = true;
	...
	if (n->item == static_cast<uint32_t>(1 << 31)) {
		for(std::set<uint32_t>::const_iterator item = t_n.begin(); item != t_n.end(); item++){
			if (n->succ->find(*item) == n->succ->end()) {
				int support = TList->supp_singleton(*item);
					if (support >= static_cast<int>(minSupp)) {
					if (root->clos->support > static_cast<uint32_t>(support)) {
...
void print_concept_as_set(std::set<uint32_t>* itemset) {
		printf("{");
		std::set<uint32_t>::iterator newciit = itemset->begin();
...
void print_concept_as_vector(std::vector<uint32_t>* itemset) {
		printf("{");
		std::vector<uint32_t>::iterator newciit = itemset->begin();
		for (; newciit != itemset->end(); ++newciit) {
				printf("%d,", *newciit);
		}
		printf("}");
...
bool is_valid_candidate(MinNode* const _parent_node, const uint32_t _item, vector<uint32_t>* const _fid_out, MinNode* const _root) {
	...
	for (uint32_t i = _parent_node->size + 1; i != 0; --i) {
		...
		for (uint32_t j = _parent_node->size + 1; j != 0; --j) {
			if (j != i) {
				if (j == (_parent_node->size + static_cast<uint32_t>(1))) {
...
void extractERFromLattice(GenLatticeNode* node, const std::string& output_file) {
	...
	if (!node->generator.empty() && !node->closure->deleted) {
		std::set<uint32_t> consequent;
		std::set_difference(node->closure->itemset.begin(), node->closure->itemset.end(), node->generator.begin(), node->generator.end(), std::inserter(consequent, consequent.end())
		);
		...
		if (!consequent.empty()) {
			...
			for (auto it = node->generator.begin(); it != node->generator.end(); ++it) {
				out << *it;
				if (std::next(it) != node->generator.end()) out << ",";
			}
			out << "} => {";
			for (auto it = consequent.begin(); it != consequent.end(); ++it) {
				out << *it;
				if (std::next(it) != consequent.end()) out << ",";
			}
			out << "} (support: " << node->closure->support << ", confidence: 1.0)\n";
		}
	}

	for (GenLatticeNode* child = node->fils; child; child = child->frere) {
		extractERFromLattice(child, output_file);
	}

	if (node->parent == nullptr && !firstCall) {
		out.close();
	}
}
...
void extractIRFromLattice(GenLatticeNode* latticeRoot, float minconf, const std::string& output_file) {
	...
	out << "=== Informative Rules (IR) ===\n";

	std::vector<GenLatticeNode*> stack;
	if (latticeRoot) {
		stack.push_back(latticeRoot);
	}

	while (!stack.empty()) {
		GenLatticeNode* node = stack.back();
		stack.pop_back();

		ClosedIS* G_closure = node->closure;
		if (G_closure->deleted) {
			for (GenLatticeNode* child = node->fils; child; child = child->frere) {
		...
		uint32_t supp_G = node->support;
		if (supp_G == 0) {
			std::cerr << "Warning: Zero support for generator { ";
			for (auto item : node->generator) std::cerr << item << " ";
			std::cerr << "}" << std::endl;
			for (GenLatticeNode* child = node->fils; child; child = child->frere) {
				stack.push_back(child);
			}
		...
		// Iterate over all successor closures of G's closure
		for (auto& succ_pair : *G_closure->succ) {
			ClosedIS* F = succ_pair.second;
			if (F->deleted) continue;

			std::set<uint32_t> F_minus_G;
			std::set_difference(F->itemset.begin(), F->itemset.end(), node->generator.begin(), node->generator.end(), std::inserter(F_minus_G, F_minus_G.begin()));

			if (!F_minus_G.empty()) {
				float confidence = static_cast<float>(F->support) / supp_G;

				if (confidence >= minconf && confidence < 1.0f) {
					out << "{";
					for (auto it = node->generator.begin(); it != node->generator.end(); ++it) {
						out << *it;
						if (std::next(it) != node->generator.end()) out << ",";
...
GenLatticeNode::GenLatticeNode(uint32_t item, std::set<uint32_t> generator, uint32_t support, ClosedIS* closure, GenLatticeNode* parent) {
	this->item = item;
	...
	this->frere = nullptr;
	this->parent = parent;
	this->equiv_next = nullptr;
	this->preds = std::set<GenLatticeNode*>();
	this->succs = std::set<GenLatticeNode*>();
}
